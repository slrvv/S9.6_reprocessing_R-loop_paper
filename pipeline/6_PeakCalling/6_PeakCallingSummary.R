...
projPath <- "/project/ChromGroup_Seq_data/External_data/S9.6_cutntag/Reprocessing"
summaryPath <- "/project/ChromGroup_Seq_data/External_data/S9.6_cutntag/Reprocessing/experiment_summary_peaks.csv"
  ...
  if(histInfo[1] != "IgG"){
    for(type in peakType){
      file <- paste0(projPath, "/peakCalling/SEACR/", hist, 
             "_seacr_norm_", type, ".peaks.relaxed.bed")
      print(file)
...
write.table(peakWidth, paste0(projPath, 
                               "/alignment/summary_peak_calling_norm_width.txt"),
            row.names = F)

peakN %>% select(Histone, Replicate, peakType, peakN)


hist_rename <- str_split(peakN$Histone, "[.]", n = 2)


peakN$GEOid <- sapply(hist_rename,"[[",1)
peakN$Histone <- sapply(hist_rename,"[[",2)
...
write.table(peakReprod, paste0(projPath, 
                                 "/alignment/summary_peak_calling_norm.txt"),
            row.names = F)
